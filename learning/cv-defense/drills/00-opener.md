...
> and an HR system. My core strength is complex React front ends, and on CloudForestX I built the Node
> and PostgreSQL side behind them as well. On the side I build and ship my own web products, live on
> their own domains, so I'm comfortable taking something from idea to production and search-ranked.
> Right now I'm after a frontend-heavy full-stack role where I can go deep on product.
...
> behind **40 cost dashboards**, and I built the **Node ingestion service** that reads EC2, EBS, S3 and
> CloudWatch across **200+ accounts through STS cross-account roles** into PostgreSQL.
>
> *Boundary, and say it here rather than waiting: "I built the surface that shows the recommendation
> and the ingestion that feeds it. I didn't write the analysis in between."*
...
- **A (40%):** ~30s. Identity → domains → honest strength → the ask. Deduct for rambling, for opening
  with a tech list instead of a story, and for either **overclaiming** the backend or **underclaiming**
  CloudForestX (it is "built," not "contributed to," since 2026-07-24).
- **B (40%):** each opener = what it is (plain) → role → one hook. **The CloudForestX and DentScribe
  ...
  **Instant fail on any banned word:** *hydration · code-splitting · TypeScript-at-Dwellworks · "I own
  ...
  that opener.
- **C (20%):** Q4 → *"I lead with the front end, and on CloudForestX I built the Node and PostgreSQL
  side too"* — one named project carrying the claim. Q5 → any project with a **specific** reason. Q6 →
  growth/scope/stack, no employer trash, no internal detail.
